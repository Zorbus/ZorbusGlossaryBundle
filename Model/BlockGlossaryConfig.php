...
namespace Zorbus\GlossaryBundle\Model;
...
class BlockGlossaryConfig extends BlockConfig
{
    ...
    public function __construct(AdminInterface $admin, FormFactory $formFactory, $httpKernel)
    {
        ...
        $this->enabled = true;
        $this->themes = array(
            'ZorbusGlossaryBundle:Glossary:default' => 'List only caracters with entries',
            'ZorbusGlossaryBundle:Glossary:all' => 'List all caracters'
            );
    ...
    public function getFormMapper()
    {
        return $this->formMapper
                        ->add('glossary', 'entity', array(
                            'class' => 'Zorbus\\GlossaryBundle\\Entity\\Glossary',
    ...
    public function getBlockEntity(array $data, BlockEntity $block = null)
    {
        ...
        $block->setCategory('Glossary');
        $block->setParameters(json_encode(array('glossary_id' => $data['glossary']->getId())));
